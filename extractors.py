...
_TERM_RULES = [
    # جذع «مكاف» يلتقط «مكافآت» (بالآلف الممدودة) و«مكافأة» معاً — كلاهما رواتب
    (r"راتب|رواتب|مكاف|payroll|salary|bonus",                   "رواتب",           None,                    False),
    # الغرامات قبل «سداد الفواتير» — «غرامة تأخير سداد» كانت تذوب في الرسوم فتختفي.
    # جذع «مخالف» يلتقط «مخالفات مرورية» الجمع (كانت «مخالفة» المفرد تفوّتها).
    (r"غرام|مخالف|penalty|late\s*fee",                          "غرامات ومخالفات", "غرامات ومخالفات",       True),
    ...
    # موردون مسمّون — كانوا يذوبون في «مصروفات أخرى» فتتضخم بلا معنى (فخ أبو فيصل)
    (r"مورد|توريد|مصنع|supplier",                                "موردون ومشتريات", None,                    False),
    (r"تأمين|insurance|تكافل",                                   "تأمين",           "تأمين",                 True),
    (r"atm\s*cash|cash\s*wd|سحب\s*نقدي",                         "سحب نقدي",        "سحوبات نقدية",          True),
...
GENERIC_SOURCES = {"إيداعات نقدية", "تحويلات بنكية", "رسوم وخدمات بنكية",
                   ...
                   "تحويل", "حوالة صادرة", "موردون ومشتريات", "تأمين",
...
_NON_OP_RULES = [
    ...
    # «تمويل - مصرف الراجحي» داخلٌ للحساب = قرض، ليس إيراداً (كان يذوب في الدخل
    # فيخفّف تركّز العميل من 60% إلى 49% — فخ أبو فيصل). البنك ليس عميلاً.
    (r"مرابحة|تورّق|تورق|إجارة\s*منتهية|تمويل\s*بنكي|تمويل من|دفعة تمويل|"
     r"تمويل\s*[-–—]|(?:مصرف|بنك).{0,30}(?:تمويل|قرض)|(?:تمويل|قرض).{0,30}(?:مصرف|بنك)|"
     r"إيداع\s*(?:قرض|تمويل)|bank financing|loan disbursement|قرض\s*جديد", "تمويل بنكي", True),
...
def _party_from_desc(desc: str, is_income: bool) -> str | None:
    ...
    # «تحويل - محمد عبدالله» → الاسم). «تحويل» المجرّدة كانت تُفلت الاسم فيضيع نصف
    # الرواتب في «تحويلات» حين يتغيّر وصف الحوالة بين الشهور — فخ أبو فيصل.
    if re.search(r"راتب|رواتب|مكاف|salary|payroll|تحويل|حوالة", parts[0], re.I) and len(parts) > 1:
        cand = parts[-1][:40].strip()
        if cand and cand not in _NOT_A_NAME:
            return cand
